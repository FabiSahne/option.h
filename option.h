...
#define OPTION(type, name)                                                     \
  ...
  static inline Option_##name option_##name##_some(type val) {                 \
    return (Option_##name){.tag = Some, .payload = val};                       \
  }                                                                            \
                                                                               \
  ...
  static inline int option_##name##_is_none(const Option_##name *o) {          \
    return o->tag == None;                                                     \
                                                                               ...
                                                                               \
  static inline int option_##name##_is_some_and(const Option_##name *o,        \
                                                int (*f)(type *)) {            \
    return o->tag == Some && f((type *)&o->payload);                           \
  }                                                                            \
                                                                               \
  static inline int option_##name##_is_none_or(const Option_##name *o,         \
                                               int (*f)(type *)) {             \
  ...
  static inline type option_##name##_unwrap(Option_##name o) {                 \
    ...
    return o.payload;                                                          \
                                                                               ...
                                                                               \
  static inline type option_##name##_unwrap_or(Option_##name o,                \
                                               type fallback) {                \
    return o.tag == Some ? o.payload : fallback;                               \
  }                                                                            \
                                                                               \
  static inline type option_##name##_unwrap_or_else(Option_##name o,           \
                                                    type (*f)()) {             \
  ...
  static inline type *option_##name##_as_ptr(Option_##name *o) {               \
    return o->tag == Some ? &o->payload : NULL;                                \
                                                                               ...
                                                                               \
  static inline Option_##name option_##name##_and(Option_##name o,             \
                                                  Option_##name other) {       \
    return o.tag == Some ? other : o;                                          \
  }                                                                            \
                                                                               \
  static inline Option_##name option_##name##_and_then(                        \
      Option_##name o, Option_##name (*f)(type)) {                             \
    return o.tag == Some ? f(o.payload) : o;                                   \
  }                                                                            \
                                                                               \
  static inline Option_##name option_##name##_or(Option_##name o,              \
                                                 Option_##name other) {        \
    return o.tag == Some ? o : other;                                          \
  }                                                                            \
                                                                               \
  static inline Option_##name option_##name##_or_else(Option_##name o,         \
                                                      Option_##name (*f)()) {  \
    return o.tag == Some ? o : f();                                            \
  }                                                                            \
                                                                               \
  static inline Option_##name option_##name##_map(Option_##name o,             \
                                                  type (*f)(type)) {           \
    return o.tag == Some ? option_##name##_some(f(o.payload)) : o;             \
  }                                                                            \
                                                                               \
  static inline Option_##name option_##name##_filter(Option_##name o,          \
                                                     int (*f)(const type *)) { \
  ...
  static inline type *option_##name##_insert(Option_##name *o, type value) {   \
    ...
    return &o->payload;                                                        \
                                                                               ...
                                                                               \
  static inline type *option_##name##_get_or_insert(Option_##name *o,          \
                                                    type value) {              \
    if (o->tag == Some)                                                        \
      return &o->payload;                                                      \
  ...
  static inline Option_##name option_##name##_take(Option_##name *o) {         \
    ...
    return copy;                                                               \
                                                                               ...
                                                                               \
  static inline Option_##name option_##name##_take_if(Option_##name *o,        \
                                                      int (*f)(type *)) {      \
    return option_##name##_is_some_and(o, f) ? option_##name##_take(o)         \
                                             : option_##name##_none();         \
  }                                                                            \
                                                                               \
  static inline Option_##name option_##name##_replace(Option_##name *o,        \
                                                      type value) {            \
    Option_##name copy = *o;                                                   \
    o->tag = Some;                                                             \
    o->payload = value;                                                        \
    return copy;                                                               \
                                                                               ...
                                                                               \
  static inline int option_##name##_eq_with(                                   \
      const Option_##name *o, const Option_##name *other,                      \
      int (*eq)(const type *, const type *)) {                                 \
    return (o->tag == Some && other->tag == Some)                              \
               ? eq((const type *)&o->payload, (const type *)&other->payload)  \
               : (o->tag == other->tag);                                       \
  }                                                                            \
                                                                               \
  static inline int option_##name##_cmp_with(                                  \
      const Option_##name *o, const Option_##name *other,                      \
