...
static inline int greater_than_one(int *x) { return *x > 1; }
...
static bool unwrap_unchecked() {
  ...
  // ASSERT(strcmp(unwrapped, "air") == 0); // MAYBE TRUE, MAYBE FALSE, MAY
  // CRASH
...
static inline int is_even(const int *n) { return *n % 2 == 0; }
...
static inline int int_cmp(const int *a, const int *b) { return *a - *b; }
